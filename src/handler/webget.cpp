...
static inline void curl_set_common_options(CURL *curl_handle, const char *url, curl_progress_data *data)
{
    ...
    // Respect configured TLS verification mode (skip_cert_verify=false by default).
    curl_easy_setopt(curl_handle, CURLOPT_SSL_VERIFYPEER, global.skipCertVerify ? 0L : 1L);
    curl_easy_setopt(curl_handle, CURLOPT_SSL_VERIFYHOST, global.skipCertVerify ? 0L : 2L);
